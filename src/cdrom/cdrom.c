...
//-- Private Variables -------------------------------------------------------
static char    cdrom_buffer[BUFFER_SIZE] __attribute__((aligned(16)));
...
//----------------------------------------------------------------------------
static void callback_DelayThread(int id, uint16 time, void *sema_id)
{
...
void delayThread(uint16 hsync)
{
	...
	SetAlarm(hsync,callback_DelayThread,(void *)sema_id);
	WaitSema(sema_id);
...
int    cdrom_load_fix_file(char *FileName, unsigned int Offset)
{
    ...
    do {
        readed = fioRead(fd,neogeo_fix_memory, BUFFER_SIZE);
        convert_all_char((neogeo_fix_memory + Offset),  readed, video_fix_usage + (Offset>>5));
        Offset += readed;
    } while( readed == BUFFER_SIZE );
...
int    cdrom_load_pat_file(char *FileName, unsigned int Offset, unsigned int Bank)
{
    ...
    /*
    for (fd =0 ; fd<Readed;fd+=2)
    	temp_buffer[fd]= cdrom_buffer[fd+1]<<8|cdrom_buffer[fd];
    */	
...
//----------------------------------------------------------------------------
/*
void obj_conv(unsigned char *src, unsigned char *dst, int len, unsigned char *usage_ptr)
{
    register int    i;
    int offset;
...
void neogeo_upload(void)
{
    ...
    case    1:    // FIX
        ...
        do {
            memset(cdrom_buffer, 0, BUFFER_SIZE);
            swab(Source, cdrom_buffer, min(BUFFER_SIZE, Taille));
            fix_conv(cdrom_buffer, Dest, min(BUFFER_SIZE, Taille),  video_fix_usage + (Offset>>5));
            Source += min(BUFFER_SIZE, Taille);
            Dest += min(BUFFER_SIZE, Taille);
            Offset += min(BUFFER_SIZE, Taille);
            Taille -= min(BUFFER_SIZE, Taille);
