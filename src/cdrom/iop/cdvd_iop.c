...
static struct _iop_device file_driver;

/* Filing-system exported functions */
void CDVD_init( struct _iop_device *driver);
...
void CDVD_init( struct _iop_device *driver)
{
	printf("CDVD: CDVD Filesystem v1.16\n");
	printf("by A.Lee (aka Hiryu) & Nicholas Van Veen (aka Sjeep)\n");
	printf("init dumb patch by O.Biot (evilo) \n");
	printf("CDVD: Initializing '%s' file driver.\n", driver->name);

	// dumb patch to avoid waiting for the drive to be ready...
	CdInit(0x01);
	
	/*
	-> init parameters
        0x00		// init cd system and wait till commands can be issused
    	0x01		// init cd system
    	*/
...
//	int copy_size;
	...
	start_sector = fd_table[i].LBA + (getSectorCount(fd_table[i].filePos));
...
static struct _iop_device_ops *filedriver_functarray;
...
int _start( int argc, char **argv)
{
	...
	filedriver_functarray->init = (void*)CDVD_init;
	filedriver_functarray->open = (void*)CDVD_open;
	filedriver_functarray->close = (void*)CDVD_close;
	filedriver_functarray->read = (void*)CDVD_read;
	filedriver_functarray->write = (void*)CDVD_write;
	filedriver_functarray->lseek = (void*)CDVD_lseek;
...
//				&& ((pathname[path_len]=='/') || (pathname[path_len]=='\\')))
	...
	CachedDirInfo.sector_num = getSectorCount(CDVolDesc.rootToc.tocSize);
...
int FindPath(char* pathname)
{
	...
	while(dirname != NULL)
	{
		...
		CachedDirInfo.sector_num = getSectorCount(localTocEntry.fileSize);
